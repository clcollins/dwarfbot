...
to use Go 1.24 and refreshes every dependency to its latest
compatible version.
...
- Go 1.26 is the latest stable release but requires a newer
  Go toolchain than is available in this environment
  (go1.24.7). Go 1.24 is the latest version we can target
